...
const Job = ({ job }) => {
  ...
  const isSaved = Array.isArray(savedJobs) && savedJobs.some(saved => saved._id === job._id);

  const daysAgoFunction = (mongodbTime) => {
    const createdAt = new Date(mongodbTime);
    const currentTime = new Date();
    const timeDifference = currentTime - createdAt;
    return Math.floor(timeDifference / (1000 * 60 * 60 * 24)); // ms -> days
  };
  ...
  return (
    <div className='p-5 rounded-md shadow-xl bg-white border border-gray-100'>
      ...
      <div className='flex items-center gap-4 mt-4'>
        ...
        <Button
          className={`bg-[#7209b7] ${isSaved ? 'opacity-60' : ''}`}
          onClick={() =>
            isSaved ? dispatch(removeSavedJob(job._id)) : dispatch(addSavedJob(job))
          }
          disabled={isSaved}
        >
